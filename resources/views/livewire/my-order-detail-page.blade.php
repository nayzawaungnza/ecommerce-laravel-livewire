            ...
            <div class="p-4 md:p-5 flex gap-x-4">
                ...
                <div class="grow">
                    ...
                    <div class="mt-1 flex items-center gap-x-2">
                        ...
                        case 'pending':
                        $paymentStatus = 'bg-orange-500';
                        break;
                        case 'paid':
                        $paymentStatus = 'bg-green-500';
                        break;

                        case 'failed':
                        $paymentStatus = 'bg-red-500';
                        break;
                        default:
                        $paymentStatus = '';
                        break;
                        ...
                        @endphp
                        <span class="{{$paymentStatus}} py-1 px-3 rounded text-white shadow"> {{$order->payment_status}}</span>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <tr wire:key="{{$orderItem->id}}">
                            <td class="py-4">
                                <div class="flex items-center">
                                    <img class="h-16 w-16 mr-4" src="{{url('storage/', $orderItem->product->images[0])}}" alt="{{$orderItem->product->name}}">
                                    <span class="font-semibold">{{$orderItem->product->name}}</span>
                                </div>
                            </td>
                            <td class="py-4">{{Number::currency($orderItem->unit_amount, 'MMK ')}}</td>
                            <td class="py-4">
                                <span class="text-center w-8">{{$orderItem->quantity}}</span>
                            </td>
                            <td class="py-4">{{Number::currency($orderItem->total_amount, 'MMK ')}}</td>
                        </tr>
                        @endforeach
        ...
        <div class="md:w-1/4">
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="flex justify-between mb-2">
                    <span>Subtotal</span>
                    <span>{{Number::currency($order->grand_total, 'MMK ')}}</span>
                </div>
                <div class="flex justify-between mb-2">
                    <span>Taxes</span>
                    <span>{{Number::currency(0, 'MMK ')}}</span>
                </div>
                <div class="flex justify-between mb-2">
                    <span>Shipping</span>
                    <span>{{Number::currency(0, 'MMK ')}}</span>
                ...
                <div class="flex justify-between mb-2">
                    ...
                    <span class="font-semibold">{{Number::currency($order->grand_total, 'MMK ')}}</span>
                </div>
